...
一个自带仿人类长期记忆和 RAG 知识库的多模型 AI 聊天平台，支持 AI 自主调用外部工具（Function Calling），基于 Spring Boot 4 + React 19 构建。在线运行的完整 SaaS 平台，已承载日均 200-500 活跃用户。
...
**与其他框架的关键差异**：CrewAI / Mem0 / LangChain 等主流框架要么不支持遗忘衰减，要么依赖定时任务轮询。HanaChat 独创**懒衰减**机制——读取时才检查时间、执行衰减，零空闲开销。在所有对比框架中，这是最接近人类记忆模型的实现。
...
HanaChat 历经六个大版本迭代，从基础 AI 聊天逐步进化为具备长期记忆和自主工具调用能力的成熟平台：

| 版本 | 主题 | 核心交付 |
|------|------|----------|
| **v1.0** | 基础设施 | 用户系统（注册/登录/JWT/邮箱验证码）、SSE 流式聊天、多模型切换、联网搜索（千帆）、提示词社区、后台管理系统、Token 计费与赞助审核 |
| **v2.0** | 核心智能 | **长期记忆系统**（四级衰减 + 懒衰减 + 对话摘要）、**RAG 知识库**（ChromaDB + bge-large-zh 嵌入）、前端 React 重构（Vite + Tailwind）、移动端适配 |
| **v3.0** | 健康加固 | 全项目健康审查（31 个问题）、数据库索引优化（全表扫描 6→0）、Flyway 数据库迁移引入、Agent 设计决策、技术债务清理 |
| **v4.0** | 安全 & 工具 | **环境变量管理重构**（46 处 @Value → 13 个 ConfigurationProperties）、**AES-128-GCM API Key 加密**、**工具调用重构**（上下文注入→Function Calling）、22 条精细化限流、管理后台审计日志、持久登录 |
| **v5.0** | 记忆深化 | **混合检索**（记忆系统：向量+BM25+知识图谱三路召回+RRF 融合+Cross-Encoder Rerank）、**知识图谱**（实体/关系/时态/消歧/图扩展）、**提示词级记忆隔离**、记忆安全加固（23 项风险修复）、移除 PDF/DOCX 解析（实测质量不达标） |
| **v6.0** | 生产优化 | 性能优化（移除知识库 BM25 双索引释出 80-120MB、虚拟线程、数据库连接池收紧）、Tool Loop 限为 1 轮（保障回复速度）、5 项生产问题修复（Cloudflare/CSP/CORS/Spring Security 7 兼容）、8 项运行时 Bug 修复、角色扮演体验优化（摘要角色化、Tool 结果角色化指令）、提示词级上下文隔离下沉到会话层 |

---

## 安全体系

HanaChat 经过三轮安全审查（v3.0 健康审查、v4.0 安全评估、v6.0 代码安全审查），已修复全部 Critical/High 级别问题。
